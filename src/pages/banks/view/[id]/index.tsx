...
import { deleteLoanById } from 'apiSdk/loans';
...
function BankViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BankInterface>(
    ...
    () =>
      getBankById(id, {
        relations: ['user', 'loan', 'transaction'],
      }),
  );

  const loanHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('loan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/loans/create?bank_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.loan?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/loans/view/${record.id}`)} key={record.id}>
                            <Td>{record.amount}</Td>
              ...
              {hasAccess('transaction', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/transactions/create?bank_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
